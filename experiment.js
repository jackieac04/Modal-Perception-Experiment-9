...
function jsonToCsv(items) {
  // submit button may not be working if window.frame is null - please check
...
function redirect() {
  ...
  window.location =
    "https://jhu.sona-systems.com/webstudy_credit.aspx?experiment_id=754&credit_token=2d6ab745370a4d0ba5567cdfdef69ee9&survey_code=" +
    window.subjectID;
}
...
function postData() {
  ...
  $.post(
    "http://pclpsrescit2.services.brown.edu/blt_lab/mp-7/data/studysave.php",
    {
  ...
  $("#instructions").show();
  $("#instructions").text(
    "Thank you! Please wait while your secret code is being generated. This may take up to 5 minutes..."
  );
}
...
class Ball {
  ...
  updatePosition(trial, trialVal) {
    if (trial[trialVal].trialType === "OSPB") {
      ...
      if (this.x < halfCanvasWidth) {
        //if x pos is left side
        this.x = this.x + velX;
        if (trial[trialVal].diskLocation === "bottom") {
          //if trial type is bottom
          this.y = this.y + 1.6;
        } else {
          //if trial type is top
          this.y = this.y - 1.1;
        }
      } else {
        //if x pos is middle or right
      ...
      if (this.x < halfCanvasWidth) {
        ...
        if (
          AWidth + dotRadius / 4 <= this.x &&
          this.x < AWidth + dotRadius / 2
        ) {
          this.y = this.y + 15;
        }
        if (
          AWidth + dotRadius / 2 <= this.x &&
          this.x < AWidth + 1.5 * dotRadius
        ) {
          this.y = this.y - 3;
        }
        if (
          AWidth + 1.5 * dotRadius <= this.x &&
          this.x < AWidth + 2 * dotRadius
        ) {
          this.y = this.y;
        }
        if (
          AWidth + 2 * dotRadius <= this.x &&
          this.x < AWidth + 3 * dotRadius
        ) {
          this.y = this.y - 3;
        }
  ...
  updateCongruence(trial, trialVal) {
    return new Promise((resolve) => {
      if (trial[trialVal].trialType === "OSPB") {
        if (trial[trialVal].spatiotemporalType === "incongruent") {
          //if trial type is incongruent
          if (trial[trialVal].diskLocation === "bottom") {
            //if trial type is bottom
            this.y = 93; //set top
          } else {
...
class Occluder {
  ...
  updatePosition() {
    if (this.y < 51) {
      //51 is the pos of the top occluder
...
function generateNewBallsHelper(x, y) {
  let ballA = new Ball(x, y, "white", dotRadius);
  return ballA;
}
...
function generateNewOccluder(height, y) {
  let occluder = new Occluder(height, y);
  return occluder;
}
/* 
...
function style(type, trial, trialVal) {
  ...
  if (trial[trialVal].trialType === "OSPB") {
    ...
    occluderB = generateNewOccluder(100, halfCanvasHeight - 95);
    occluderA.drawOccluder();
    occluderB.drawOccluder();
  } else {
    occluderD = generateNewOccluder(100, halfCanvasHeight - 225); //first occluder
    occluderE = generateNewOccluder(100, halfCanvasHeight - 97.5); //after it splits in 2
...
async function stimuliPreview(trial, trialVal) {
  // the phases before the disks and shapes move
...
function animate(trial, trialVal) {
  // make the disks and the shapes move together and occluders move off screen
  myTimeout = setTimeout(async function () {
    ...
    if (
      trainingTrial === trialsInfoTraining.length &&
      curTrial < trialsInfo.length
    ) {
    ...
    if (refresh < 76) {
      ...
      //after this period, occluder becomes two occluders (MODAL) then move offscreen (OSPB + MODAL)
...
function shapeAppearance(trial, trialVal) {
  if (trial[trialVal].trialType === "OSPB") {
    //if the trial is OSPB
    if (
      (trial[trialVal].diskLocation === "top" &&
        trial[trialVal].spatiotemporalType === "congruent") ||
      (trial[trialVal].diskLocation === "bottom" &&
        trial[trialVal].spatiotemporalType === "incongruent")
    ) {
      //if the disk should be at the top of the screen after it goes behind the occluder
